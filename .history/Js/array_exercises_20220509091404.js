...
function capitalizeWord(word = "") {
  ...
  let newWord = word.trim().toLowerCase().charAt(0); // in hoa chữ cái đầu
  let otherWord = word.trim().toUpperCase().slice(1); // copy từ chữ cái thứ 2 qua biến khác
  return `${newWord}${otherWord}`;
}
function capitalizeStr(str) {
  if (!str) return null;
  const result = str.split(" ").map(capitalizeWord);
  return console.log(result);
}
capitalizeStr("My name is Thiệp");
